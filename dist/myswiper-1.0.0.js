...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.myswiper = factory());
    ...
    function Swiper(id) {
      ...
      this.ul = this.sp.getElementsByClassName("ul")[0]; //指标总元素

      this.bp = this.sp.getElementsByClassName("b-p")[0]; //初始化进入出去事件
    ...
    Swiper.prototype.initDom = function () {
      ...
      if (this.sp.getElementsByClassName("ol").length == 0) {
        var ol = document.createElement('b-p');
    ...
    Swiper.prototype.initEvent = function () {
      ...
      this.initPointEvent(); //初始化进入及出去事件
    ...
    Swiper.prototype.initPointEvent = function () {
      ...
      this.sp.addEventListener('click', function (event) {
        var currentNode = event.target;
        var rc = event.currentTarget;
        rc.getElementsByClassName("ul")[0]; //事件代理写法
    ...
    Swiper.prototype.goPage = function (page) {
      ...
      var currentNode = this.bp.children[this.page - 1];
      [].slice.call(this.bp.children).forEach(function (item) {
        item.classList.remove("at");
      });
      var index = [].indexOf.call(currentNode.parentNode.children, currentNode);
      var p = -100 * index;
    ...
    Swiper.prototype.add = function (id, html, callback) {
      //判断是否要添加指示
      var len = this.ul.children.length;
      var addPoint = false; //判断是否要添加指示园圈，如果现在刚好不用添加，那添加一个元素之后肯定是要添加的
...
//# sourceMappingURL=myswiper-1.0.0.js.map
